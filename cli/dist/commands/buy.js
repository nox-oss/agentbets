...
export const buyCommand = new Command('buy')
    ...
    if (/^\d+$/.test(outcome)) {
        ...
        outcomeIndex = market.outcomes.findIndex(o => o.toLowerCase() === outcome.toLowerCase());
        if (outcomeIndex === -1) {
    ...
    console.log(chalk.blue('🛒 Buy Order\n'));
    ...
    if (options.dryRun) {
        ...
        console.log(chalk.gray('\nWould execute buy instruction with:'));
    ...
    try {
        const txSignature = await client.buyShares({
            marketId,
        ...
        console.log('');
        console.log(chalk.green(`✅ Bought ${shares} shares`));
